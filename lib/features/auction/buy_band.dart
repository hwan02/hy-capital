  ...
  String get why => switch (this) {
        ...
        BuyBand.early =>
          '모아 = 관리계획 수립·주민공람 때 산다. '
              '신통 = 선정 이후 ~ 기획 확정 전(아카이브 «기획중» 포함)에 산다.',
        BuyBand.late_ =>
...
///   1 동의서징구시작 ┐
///   2 동의서달성     ┘ 오르는 중
///   3 신규 선정         🟢 매수 A  ← 선정됐고 확정 신호 없음
///   4 기획 중           🟢 매수 A  ← 서울시 신통 아카이브의 «기획중»
...
///
/// 매수적기 = «선정 이후 ~ 확정 전» 전부(사용자 확인 2026-09-23). 3 이
/// 부풀어 보였던 건 «이미 확정됐는데 3 에 남은» 구역 때문이라, 판정을 좁히지
/// 않고 확정을 제대로 가려낸다 — sync_sin_archive.py 가 아카이브 기획완료 ·
/// 정비몽땅 「가이드라인 수립·구역지정·시행자지정」을 보고 5 이상으로 올린다.
BuyBand bandOfSinStage(int stage) => switch (stage) {
      3 || 4 => BuyBand.early,
      7 => BuyBand.late_,
      1 || 2 || 5 || 6 => BuyBand.rising,
      >= 8 => BuyBand.blocked,
      _ => BuyBand.unknown,
    };
...
String sinPlan(BuyBand b) => switch (b) {
      BuyBand.early => '선정~확정 전(기획중)에 사서 → 기획 확정(기획완료) 후 판다',
      BuyBand.late_ => '조합 동의서 징구 중 사서 → 조합설립인가 직전 판다',
      BuyBand.rising => '이미 확정됐거나 선정 전 — 다음 매수 자리(동의서 징구)를 기다린다',
      _ => '조합설립인가 지남 — 양도 제한·승계 불가',
    };
