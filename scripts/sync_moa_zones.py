...
#   모아 1 동의서징구시작 2 동의서달성 3 후보지선정 4 관리계획수립
#        5 주민공람14일 6 통합심의 7 관리계획승인고시 8 조합동의서징구
#        9 조합설립인가 …  ← 교안 「후계공통승」 세부 절차 그대로
...
MOA_STAGE = {
    '대상지선정': (3, '후보지 선정 발표로 오른 구간'),
    '수립범위 자문': (4, '관리계획 수립 — 수립범위 자문'),
    '사전자문': (4, '관리계획 수립 — 전문가 사전자문'),
    # 포털에 「공람」 단계가 따로 없다. 공람 중인 곳은 열람공고로 잡는다
    # (scripts/moa_notice.py) — 여기서는 5번 칸을 비워 둔다.
    '위원회심의': (6, '통합심의 중 — 고시 임박'),
    '관리지역고시': (7, '관리계획 승인고시 완료 — 조합 동의서 걷기 시작'),
}

SIN_STAGE = {
    '대상지선정': (3, '선정 발표로 오른 구간'),
    '기획완료': (5, '기획 완료 — 지정고시 대기 (첫 골짜기)'),
    # 열람공고(주민공람)는 정비계획을 «지정 전»에 공람하는 단계다.
    # 6(정비구역 지정고시)에 꽂으면 확정이 난 것처럼 보여 한 칸 앞서간다.
    # 기획이 끝나 지정을 기다리는 상태이므로 5 가 맞다 — 다만 지정이
    # 임박했다는 건 출처 문구로 남긴다.
    '열람공고': (5, '열람공고 — 정비구역 지정 임박 (확정 전)'),
...
def main():
    ...
    if dry:
        ...
        print('\n★ 매수 A (저점) 후보')
        for w in want:
            a = (w['stage'] in (4, 5)) if w['kind'] == '모아타운' \
                else (w['stage'] in (4, 5))
            if a:
