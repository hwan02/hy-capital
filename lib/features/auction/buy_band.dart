...
///   5 기획 완료(확정)   🟢 매수 A  ← 지정고시 직전 — 첫 골짜기
///   6 정비구역 지정고시  오르는 중 (확정으로 튄다 — 추격 매수는 늦다)
///   7 동의서 징구       🟡 매수 B  ← 두 번째 골짜기
///   8+ 조합설립인가↑    🚫 양도 제한 · 승계 불가
///
/// 3(선정)이 전엔 「오르는 중」이었다 — 교안 그래프(선정 발표로 한 번 튄다)를
/// 따른 것인데, 본인 전략(자료실 「신통 타이밍」 2026-09-05)은 «매수 창 =
/// 대상지 선정 ~ 확정 사이»다. 칩 라벨도 「대상지선정 후~확정 전」이라고
/// 써놓고 판정만 선정을 빼고 있었다. 게다가 포털엔 「기획 중」 상태가 없어
/// 신통 구역 대부분이 3 에 머문다 — 매수적기에 거의 안 나왔다.
/// 선정 직후 바로 사지 말라는 건 «트래킹»의 문제라 why 문구로 남긴다.
BuyBand bandOfSinStage(int stage) => switch (stage) {
      3 || 4 || 5 => BuyBand.early,
      7 => BuyBand.late_,
      1 || 2 || 6 => BuyBand.rising,
      >= 8 => BuyBand.blocked,
      _ => BuyBand.unknown,
    };
