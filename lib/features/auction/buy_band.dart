...
//   • 신통기획 — 서울시 신통 아카이브에 «기획중»(선정~기획 확정 전)일 때 «산다»
//                → 기획 확정(아카이브 기획완료) 후 «판다». (09-23 사용자 확인)
//
// 공통 — «조합설립인가가 나면 끝»이다.
...
extension BuyBandInfo on BuyBand {
  ...
  String get why => switch (this) {
        BuyBand.rising =>
          '발표로 값이 튄 구간. 모아는 통합심의 전 골짜기에서, 신통은 '
              '아카이브 «기획중»으로 확인되면 산다(선정만 된 곳은 트래킹).',
        BuyBand.early =>
          '모아 = 관리계획 수립·주민공람 때 산다. '
              '신통 = 서울시 신통 아카이브 «기획중»(선정~기획 확정 전)일 때 산다.',
        BuyBand.late_ =>
...
///   1 동의서징구시작 ┐
///   2 동의서달성     ┘ 오르는 중
///   3 신규 선정         트래킹     ← 포털 「대상지선정」만 — 실제 상태를 모른다
///   4 기획 중           🟢 매수 A  ← 서울시 신통 아카이브의 «기획중» = 매수적기
///   5 기획 완료(확정)   오르는 중  ← 확정으로 튄다. 추격 매수는 늦다
///   6 정비구역 지정고시  오르는 중
///   7 동의서 징구       🟡 매수 B  ← 두 번째 골짜기
///   8+ 조합설립인가↑    🚫 양도 제한 · 승계 불가
///
/// 매수적기 = «선정 ~ 확정 전». 여기서 «확정»은 신속통합기획 «기획 확정»
/// (가이드라인 확정, 아카이브에 기획완료 날짜·링크가 붙는 때)다 — 정비구역
/// 지정고시가 아니다. 아카이브 표에서 「기획중 · -」인 구역이 매수적기다.
/// (사용자 확인 2026-09-23. 전엔 선정을 빼고 기획완료를 넣어 거꾸로였다.)
/// 선정 직후 바로 사지 말라는 건 «트래킹»의 문제라 why 문구로 남긴다.
/// 아카이브는 확정 뒤 늦게 바뀐다 — sync_sin_archive.py 가 기사로 «확인
/// 필요»를 띄우고, 판단은 서울시보·정비몽땅으로 사람이 한다.
///
/// 3 을 매수 A 에서 뺀 이유(2026-09-23): 3 은 포털 「대상지선정」에서만 온
/// 값이라 실제로 기획중인지, 이미 확정됐는지, 아카이브에 없는 아파트
/// 재건축인지 모른다. 3·4 를 다 넣었더니 신통 매수적기가 100곳을 넘었다 —
/// 아카이브 «기획중»은 23곳이다. 4 는 sync_sin_archive.py 가 아카이브로
/// 확인했을 때만 붙는다.
BuyBand bandOfSinStage(int stage) => switch (stage) {
      4 => BuyBand.early,
      7 => BuyBand.late_,
      1 || 2 || 3 || 5 || 6 => BuyBand.rising,
      >= 8 => BuyBand.blocked,
      _ => BuyBand.unknown,
    };
...
String sellLineOf(BuyBand b) => switch (b) {
      BuyBand.early =>
        '모아 = 통합심의 통과 직후. 신통 = 기획 확정(기획완료) 후. 늦어도 조합설립인가 전.',
...
String sinPlan(BuyBand b) => switch (b) {
      BuyBand.early => '아카이브 «기획중»일 때 사서 → 기획 확정(기획완료) 후 판다',
      BuyBand.late_ => '조합 동의서 징구 중 사서 → 조합설립인가 직전 판다',
      BuyBand.rising => '선정만 됐거나 이미 확정 — 아카이브 «기획중»이 뜰 때까지 트래킹',
      _ => '조합설립인가 지남 — 양도 제한·승계 불가',
    };
...
const kSinStageDoing = <int, String>{
  1: '동의서 걷는 중',
  2: '동의율 달성 · 선정 대기',
  3: '선정 — 아카이브 «기획중» 확인 전(트래킹)',
  4: '기획 «수립» 중 — 매수적기(선정~확정 전)',
  5: '기획 «확정» — 보유 중이면 매도 구간',
  6: '지정고시 완료 · 동의서 준비',
  7: '조합 동의서 «징구» 중',
  8: '조합 운영',
...
/// 카드에 띄우는 «다음에 할 일» 한 줄 — 단계마다 «언제 파는지»가 다르다.
///
/// 전엔 모든 단계에 「다음 상승: ○○ 직전 매도」를 붙였다. 그러면
///   · 모아 첫 골짜기는 「통합심의 «직후»」에 파는데 «직전»이라고 나왔고,
///   · 신통은 「기획 확정 «후»」에 파는데 「정비구역 지정고시 직전」이라고 나왔다.
/// «직전»이 맞는 건 두 번째 골짜기(조합설립인가)뿐이다 — 인가가 나면 승계가
/// 막혀 못 판다. (자료실 「가격이 뛰는 구간」 08-31 + 사용자 확인 09-23)
const kMoaNextMove = <int, String>{
  3: '다음 매수: 관리계획 수립·주민공람',
  4: '매도: 통합심의 통과 직후',
  5: '매도: 통합심의 통과 직후',
  6: '통합심의 중 — 보유 중이면 통과 직후 매도',
  7: '매도: 조합설립인가 직전 (넘기면 승계 불가)',
  8: '매도: 조합설립인가 직전 (넘기면 승계 불가)',
};

/// 신통 버전. 매수적기 = 아카이브 «기획중»(4) → 기획 확정(5) 후 판다.
const kSinNextMove = <int, String>{
  3: '다음 매수: 아카이브 «기획중» 확인 뒤',
  4: '매도: 기획 확정(기획완료) 후',
  5: '기획 확정 — 보유 중이면 지금이 매도 구간',
  6: '다음 매수: 조합 동의서 징구',
  7: '매도: 조합설립인가 직전 (넘기면 승계 불가)',
};
